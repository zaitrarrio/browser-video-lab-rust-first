import {encodePrompt,hashPrompt,loadTextEncoder,manifest,normal,ort,rgbaFromNchw,session,type TextEncoder} from './common';
...
// denoiser (shared with the Rust/WASM student via `encodePrompt`). The student
// is distilled with text_width matching this encoder, so its input Linear is the
// learned projection into the model width. When no text_encoder is shipped the
// embeds are seeded from the prompt so runs stay deterministic, the tab still
// works before real weights exist, and the prompt still influences the output.
// No VAE is exported, so — like the Rust/WASM student — the first three latent
// channels are shown directly.
type Latent={channels:number;frames:number;seq:number;text_width:number};
export class OnnxStudentRuntime{private m!:Awaited<ReturnType<typeof manifest>>;private denoiser!:ort.InferenceSession;private enc?:TextEncoder;
 async load(url:string,progress:(s:string)=>void){this.m=await manifest(url);if(this.m.models.text_encoder){progress('Loading text encoder…');this.enc=await loadTextEncoder(this.m)}progress('Loading student denoiser…');this.denoiser=await session(this.m.models.denoiser);progress(this.enc?'ONNX student ready · prompt-conditioned':'ONNX student ready · seeded embeds')}
 private async embed(prompt:string,L:Latent,seed:number){const {data}=await encodePrompt(this.enc,prompt,L.seq,L.text_width,(seed^hashPrompt(prompt))>>>0);return new ort.Tensor('float32',data,[1,L.seq,L.text_width])}
