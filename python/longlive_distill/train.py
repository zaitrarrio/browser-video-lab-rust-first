...
def samples(cfg,device,dtype):
    if path:=cfg.get("path"):
        ...
        for file in itertools.cycle(files):
            ...
            yield item["latents"].to(device=device,dtype=dtype),item["prompt_embeds"].to(device=device,dtype=dtype)
    else:
        while True:yield torch.randn(*cfg["latent_shape"],device=device,dtype=dtype),torch.randn(*cfg["text_shape"],device=device,dtype=dtype)
def main():
    ...
    for step in range(1,int(cfg["training"]["steps"])+1):
        lat,prompt=next(stream);time=torch.randint(0,1000,(lat.shape[0],),device=device);noisy=lat+torch.randn_like(lat)*(time.float()/1000).view(-1,1,1,1,1)
        with torch.no_grad():target=teacher(noisy,time,prompt)
        pred=student(noisy,time,prompt);loss,metrics=distillation_loss(pred,target,cfg["loss"]);opt.zero_grad(set_to_none=True);loss.backward();torch.nn.utils.clip_grad_norm_(student.parameters(),cfg["training"].get("grad_clip",1.0));opt.step()
